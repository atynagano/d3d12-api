...
pub trait ID3D12Object: IUnknown {
	...
	fn SetPrivateData(&self, guid: &GUID, data: Option<&[u8]>, ) -> Result<(), HResult> {
		let vt = self.as_param();
		let f: extern "system" fn(Param<Self>, guid: &GUID, data_size: u32, data: *const u8, ) -> HResult
			= unsafe { transmute(vt[4]) };
		let ret = f(vt, guid, data.len() as u32, data.as_ptr_or_null(), );
		ret.ok()
	}

	fn SetPrivateDataInterface(&self, guid: &GUID, data: Option<&Unknown>, ) -> Result<(), HResult> {
		let vt = self.as_param();
		let f: extern "system" fn(Param<Self>, guid: &GUID, data: Option<VTable>, ) -> HResult
			= unsafe { transmute(vt[5]) };
		let ret = f(vt, guid, option_to_vtable(data), );
		ret.ok()
	}
	...
	fn SetName(&self, name: &str, ) -> Result<(), HResult> {
		...
		let ret = f(vt, name.to_utf16().as_ptr_or_null(), );
		ret.ok()
	}
}
