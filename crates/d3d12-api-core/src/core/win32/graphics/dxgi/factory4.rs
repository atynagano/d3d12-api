...
pub trait IDxgiFactory4: IDxgiFactory3 {
	...
	fn EnumAdapterByLuid<T: IUnknown>(&self, adapter_luid: Luid, ) -> Result<(T), HResult> {
		...
		let f: extern "system" fn(Param<Self>, adapter_luid: Luid, riid: &GUID, _adapter: &mut Option<Unknown>, ) -> HResult
			= unsafe { transmute(vt[26]) };
		let ret = f(vt, adapter_luid, T::IID, &mut _adapter, );
		if ret.is_ok() {
	...
	fn EnumWarpAdapter<T: IUnknown>(&self, ) -> Result<(T), HResult> {
		let vt = self.as_param();
		let mut _adapter: Option<Unknown> = None;
		let f: extern "system" fn(Param<Self>, riid: &GUID, _adapter: &mut Option<Unknown>, ) -> HResult
			= unsafe { transmute(vt[27]) };
		let ret = f(vt, T::IID, &mut _adapter, );
		if ret.is_ok() {
			if let (Some(_adapter)) = (_adapter) {
				return Ok((T::from(_adapter)));
			}
