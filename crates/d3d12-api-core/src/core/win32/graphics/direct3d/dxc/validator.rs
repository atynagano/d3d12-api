...
impl Com for DxcValidator {
	fn vtable(&self) -> VTable { self.0.vtable() }
}

impl DxcValidator {
	pub fn Validate(&self, shader: &DxcBlob, flags: u32) -> Result<DxcOperationResult, HResult> {
		unsafe {
			let vt = self.as_param();
			let mut result_out_: Option<DxcOperationResult> = None;
			let f: extern "system" fn(Param<Self>, VTable, u32, *mut c_void) -> HResult
				= transmute(vt[3]);
			let _ret_ = f(vt, shader.vtable(), flags, transmute(&mut result_out_));
...
impl From<UnknownWrapper> for DxcValidator {
